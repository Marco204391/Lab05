...
public class AnagrammiController {
    ...
    @FXML // fx:id="txtAnagrCorretti"
    private TextArea txtAnagrCorretti; // Value injected by FXMLLoader

    @FXML // fx:id="txtAnagrErrati"
    private TextArea txtAnagrErrati; // Value injected by FXMLLoader
    ...
    void doCalcola(ActionEvent event) {
    	String text=txtParola.getText();
    	for( String c : model.findPermutations(text)){
			if(model.isCorrect(c)){
				txtAnagrCorretti.appendText(c+"\n");
			}
			else{txtAnagrErrati.appendText(c+"\n");}
		}
    }
